...
if(isset($_POST["ingreso"]))
{
	...
	{
		$coneccion=mysql_connect("localhost", "user1", "user1") or die("Error conexion".mysql_error());
		mysql_select_db("Farmacos", $coneccion) or die("Error base de datos.".mysql_error());
		
		$consulta=mysql_query("SELECT COUNT(*) FROM Productos") or die(mysql_error());
		$registro=mysql_fetch_row($consulta);

		if($registro[0]>=600) { echo "Hay demasiados registros en la Base de Datos"; die(); }
		
		$consulta=mysql_query("SELECT * FROM Productos WHERE descripcion='$valor'") or die("Error 2".mysql_error());
		$registro=mysql_fetch_row($consulta);
		
		if($registro) { echo "Ya existe tu nombre en la Base de Datos"; }
		else
		{
			mysql_query("INSERT into Productos (nombre) VALUES ('$valor')");
...
if(isset($_POST["busqueda"]))
{
	...
	if(validaBusqueda($valor))
	{
		...
		$consulta=mysql_query("SELECT descripcion FROM Productos WHERE descripcion LIKE '".$valor."%' LIMIT 0, 30");
